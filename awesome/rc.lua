...
local function run_once(cmd)
    local findme = cmd:match("([^/]+)$") or cmd
    findme = findme:gsub("%s*&%s*$", "")
    findme = findme:match("%S+")

    local check_cmd = string.format("pgrep -u $USER -x %s > /dev/null || (%s)", findme, cmd)

    awful.spawn.with_shell(check_cmd)
end
...
theme.wallpaper_path  = os.getenv("HOME") .. "/wallpapers/"
...
local function get_cpu_temp()
    ...
    if temp then
        return string.format("  %.0f°C", temp / 1000)
    end
    return "  N/A"
end
...
local function get_ram_usage()
    ...
    -- We loop through the file to find the specific lines we need
    ...
    if mem_total > 0 and mem_available > 0 then
        ...
        local used_gb = (mem_total - mem_available) / 1024 / 1024
...
weather_widget.font = theme.font;

local function update_weather_display()
    local file = io.open("/tmp/awesome_weather.txt", "r")
    if file then
        local content = file:read("*all")
        file:close()
        return content:gsub("\n", "")
...
gears.timer {
    ...
    autostart = true,
    callback  = function()
        temp_widget.markup = "<span color='#ff5555'>" .. get_cpu_temp() .. "</span>"
        ram_widget.markup = "<span color='#ff5555'>" .. get_ram_usage() .. "</span>"
    end
}
...
gears.timer {
    ...
    autostart = true,
    callback  = function()
        weather_widget.markup = "<span color='#7788af'>" .. update_weather_display() .. "</span>"
    end
}

mytextclock = wibox.widget {
    widget = wibox.widget.textclock,
    format = '<span foreground="#7788af">%d/%m/%Y   %H:%M</span>',
    font = theme.font
}
...
local function set_random_wallpaper()
    awful.spawn.with_shell("feh --bg-fill --randomize " .. theme.wallpaper_path .. "*")
end
...
wifi_widget.font = theme.font;
local function update_wifi()
    local f = io.open("/dev/shm/wifi_info", "r")
    if not f then
        wifi_widget.markup = "<span color='#ff5555'>󰤮 Disconnected</span>"
        return
    end
    ...
    local ssid, signal = content:match("^%s*(.-)%s*:%s*(%d+)")
    signal = tonumber(signal)

    local icon = "󰤨" -- Default full
    if signal < 20 then
        icon = "󰤟" -- Weak
    ...
    wifi_widget.markup = string.format(" %s%% <span>@%s</span> ", signal, ssid)
end
...
wifi_widget:connect_signal("button::press", function(_, _, _, button)
    if button == 1 then                   -- Left click
        awful.spawn("alacritty -e nmtui") -- Replace 'st' with your terminal
    end
end)
...
globalkeys = gears.table.join(
    ...
    end, { description = "launch rofi menu", group = "launcher" }),
...
clientkeys = gears.table.join(
    ...
    awful.key({ modkey }, "v", function()
        ...
        { description = "launch pavucontrol", group = "volume" })
)
...
run_once("picom &")
run_once("nm-applet &")
run_once("$HOME/.config/awesome/scripts/weather.sh &")
run_once("$HOME/.config/awesome/scripts/wifi-gather.sh &")
